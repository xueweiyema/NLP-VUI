...
from keras.layers import (BatchNormalization, Conv1D, Dense, Input, Dropout,
                          MaxPooling1D, TimeDistributed, Activation,
...
def rnn_model(input_dim, units, activation, output_dim=29):
    ...
    time_dense = TimeDistributed(Dense(output_dim), name='timedense')(bn_rnn)
    # Add softmax activation layer
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(0, recur_layers):
        simple_rnn = GRU(
            ...
            implementation=2,
            name='rnn' + str(i))(data)
        bn = BatchNormalization()(simple_rnn)
        data = bn

    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='timedense')(data)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(
        GRU(units, return_sequences=True, implementation=2),
        name='birnn')(input_data)

    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(
        Dense(output_dim), name='timedense')(bidir_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def cnn_bidirectional_rnn_model(input_dim,
                                filters,
                                kernel_size,
                                conv_stride,
                                conv_border_mode,
                                units,
                                output_dim=29):
    """ Build a recurrent + convolutional+ bidirectional network for speech 
    """
    # Main acoustic input
    input_data = Input(name='the_input', shape=(None, input_dim))
    # Add convolutional layer
...
def final_model(input_dim,
    ...
    # Main acoustic input
    input_data = Input(name='the_input', shape=(None, input_dim))
    drop_out = Dropout(0.25)(input_data)
    # Add convolutional layer
    conv_1d = Conv1D(
        filters,
        kernel_size,
    ...
    drop_out = Dropout(0.25)(conv_1d)
    # Add batch normalization
    bn_cnn = BatchNormalization(name='bn_conv_1d')(drop_out)
    for i in range(0, recur_layers):
        bidir_rnn = Bidirectional(
            GRU(units, return_sequences=True, implementation=2),
            name='birnn' + str(i))(bn_cnn)
        drop_out = Dropout(0.25)(bidir_rnn)
        bn = BatchNormalization()(drop_out)
        bn_cnn = bn
    time_dense = TimeDistributed(Dense(output_dim), name='timedense')(bn_cnn)
